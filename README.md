...
1. Ejecutar el script SQL (`ScriptBaseDatos.sql`) para crear la base de datos `prueba` y la tabla `Usuario`.  
2. Configurar la cadena de conexión en `Web.config` (clave: `Conexion`).  
3. Abrir la solución en Visual Studio (`GestionUsuarioPrueba.sln`).  
4. Configurar **Varios proyectos de inicio**:
   - Haz clic derecho sobre la solución → Propiedades → `Proyectos de inicio`.
   - Selecciona **Varios proyectos de inicio**.
   - Acción: `Iniciar` para `Presentacion.Web` y `Negocio.WCF`.  
   - Aplicar → Aceptar.
5. Ejecutar la solución (F5 o el botón verde).  
6. En el navegador abrir `Usuario.aspx` para crear usuarios.  
7. Acceder a `UsuarioConsulta.aspx` para ver, modificar o eliminar usuarios.  
...
## Contacto
Desarrollado por: Kenia Palomeque Pino
Correo: keniabpp@outlook.com